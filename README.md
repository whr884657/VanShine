...
**类型：** EdgeOne 规则引擎页整页改版（大版本）

**变更说明：**
- 「七层加速」更名为 **规则引擎**（`rules.php`），对齐腾讯云「针对特定域名生效的差异化配置」
- 删除 `l7.php`、`alias.php`，**不做**旧路径跳转或兼容页
- 规则列表：序号、拖拽排序、启用开关、编辑/复制/删除；支持搜索名称/注释
- 创建/编辑抽屉：HOST 或全站匹配 + 常用操作（压缩、强制 HTTPS、HTTP/2、节点不缓存）
- 移除别称域名后台入口（腾讯 API 尚在测试，暂不开放）
...
**类型：** EdgeOne 域名管理交互与证书状态优化（小版本）

**变更说明：**
- 添加域名后局部刷新列表，修复关闭弹窗后页面卡顿
- 添加域名只需输入子域名前缀；列表与「部署中」单行支持刷新状态
- HTTPS 证书状态区分申请中/部署中/已部署/即将过期；部署流程无整页跳转
...
**类型：** EdgeOne 弹窗尺寸与导航体验修复（小版本）

**变更说明：**
- 电脑端弹窗（添加域名、HTTPS 配置、概览筛选）尺寸对齐文件预览（约 1120×860）
- 修复 SPA fragment 双层 `#edgeoneMainContent` 嵌套；切换页面前先显示加载占位
- 新增 `--vs-desktop-modal-*` CSS 变量；修复 SPA 切换后站点表单未绑定
...
**类型：** EdgeOne 域名管理弹窗修复与 UI 规范统一（小版本）

**变更说明：**
- 修复添加域名 `OriginType` 须为 `IP_DOMAIN` 导致的 API 报错
- 电脑端弹窗对齐文件预览：居中、标题/底栏固定、内容区滚动
- 手机端抽屉统一 **80vh**，标题不再随内容滚动
- 站点标签移至选择框上方；IPv6 分段选中态加强；按钮改圆角矩形
- 全站 `--vs-mobile-drawer-height` 写入公共 CSS 与文档
...
**类型：** EdgeOne 域名管理页 UI/功能微调（小版本）

**变更说明：**
- 站点信息（接入方式、加速区域、DDoS、状态）并入站点选择板块
- 手机端：第一行站点下拉，第二行切换/添加按钮各占一半
- CNAME 点击复制；HTTPS 列增加「配置」抽屉（免费证书申请/验证/部署）
- DDoS 防护值中文化；IPv6 改为分段选择；源站类型静态展示
- 板块间距、字体与电脑端抽屉样式统一
...
**类型：** EdgeOne 域名管理页整页改版（大版本）

**变更说明：**
- 域名加速更名为域名管理；顶部域名配置 + 官方式列表
- 站点选择/切换/添加域名同一行；添加与编辑改为抽屉
- 展示 CNAME、源站、DDoS/IPv6、HTTPS；停用后才可删除
- Pages 站点禁用自助操作；从站点列表跳转自动绑定站点
...
**类型：** EdgeOne 导航与交互微调（小版本）

**变更说明：**
- 站点列表操作统一跳转至 **域名加速**、**七层加速**、**安全策略**（非独立新页）
- 手机端站点列表改为卡片布局，告别横向滑动表格
- 手机导航「概览」取消多余中折叠
- 从站点列表切换站点后整页加载，正确显示站点选择器
...
**类型：** EdgeOne 站点管理页整页改版（大版本）

**变更说明：**
- 站点数据概览：总流量/带宽峰值/请求数/防护命中 KPI + 指标折线图 Tab
- 官方式站点列表：套餐映射、状态、接入方式、搜索与快捷操作
- 新增站点改为抽屉，仅展示未绑定站点的套餐
- 本页移除站点选择器、验证信息与 CNAME 配置状态
...
**类型：** EdgeOne 概览功能级改版（次版本）

**变更说明：**
- L7 访问流量多维 Tab + TOP 图例多折线；移除访问区域分布板块
- 侧栏 6 项 KPI（含 L7 防护命中次数、sparkline、总和/峰值/均值）
- 自定义筛选改为手机抽屉 / 桌面弹窗，枚举复选框网格
...
**类型：** EdgeOne 概览 UI 微调（小版本）

**变更说明：**
- 自定义筛选弹层 + 标签，不再堆叠多行
- 区域分布纯 SVG 环形图（已在本版后续移除）
- KPI SVG 图标与区间/均值标注
...
**类型：** EdgeOne 概览全面改版（大版本）

**变更说明：**
- 概览支持 Filters.N 自定义筛选（国家/状态码/协议/缓存等 20+ 项）
- 腾讯云式布局：侧栏 KPI + 流量图 + 区域分布 + Top 排行
- 时间范围 Tab 快捷选择
...
**类型：** EdgeOne 概览体验优化（小版本）

**变更说明：**
- 时间范围精简为 30 分钟、1/6 小时、今天/昨天、3/7/14/30 天
- 概览新增基础数据 KPI（总流量、响应/请求流量、缓存命中率）
- 查询按钮与筛选条件同一行，移除多余提示
...
**类型：** EdgeOne 查询条件优化（小版本）

**变更说明：**
- 移除时间颗粒度筛选，API 自动推算 Interval
- 概览查询条件：电脑端单行横排，手机端 2 列网格
