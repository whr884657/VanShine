...
- **手机端（≤767px）**：所有底部抽屉高度统一为 **`80vh`（视口 4/5）**，CSS 变量 `--vs-mobile-drawer-height`（定义于 `assets/css/common.css`）
- **电脑端（≥768px）**：表单/配置类弹窗居中显示，标题与底部按钮固定，**仅内容区滚动**
- **按钮**：抽屉/弹窗内操作按钮使用 `vs-btn--rect`（圆角 8px），避免过圆胶囊形
