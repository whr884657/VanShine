...
if (vs_edgeone_is_ready()) {
    try {
        ...
        if ($zoneId !== '') {
            $f = $eo->function->describeFunctions(array('ZoneId' => $zoneId, 'Offset' => 0, 'Limit' => 50));
            $functions = isset($f['Functions']) ? $f['Functions'] : array();
            $kv = $eo->edgeKv->describeEdgeKVNamespaces(array('ZoneId' => $zoneId));
            $namespaces = isset($kv['Namespaces']) ? $kv['Namespaces'] : array();
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
vs_admin_layout_start('EdgeOne 边缘函数', 'cdn_edgeone_edge');
?>
