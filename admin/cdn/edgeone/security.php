...
if (vs_edgeone_is_ready()) {
    try {
        ...
        if ($zoneId !== '') {
            $security = $eo->security->describeSecurityPolicy(array('ZoneId' => $zoneId, 'Entity' => '@ZoneLevel@domain'));
            $cResp = $eo->certificate->describeDefaultCertificates(array('ZoneId' => $zoneId));
            $certs = isset($cResp['DefaultCertificates']) ? $cResp['DefaultCertificates'] : array();
        }
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
vs_admin_layout_start('EdgeOne 安全证书', 'cdn_edgeone_security');
?>
            ...
            <table class="vs-table">
                ...
                <?php foreach ($certs as $c): ?>
                    <tr>
                        ...
                        <td><?php echo vs_e(isset($c['Status']) ? $c['Status'] : '-'); ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
