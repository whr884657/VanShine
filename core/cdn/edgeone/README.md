...
| 手机端 | **总折叠抽屉**（默认一条）；内为 **中折叠大类**（总览、站点与域名…）；中折叠下为 **小类链接**；跳转后总折叠自动收起 |

| 分组 | 页面 |
|------|------|
| 总览 | 概览 |
| 站点与域名 | 站点管理、域名管理、规则引擎、DNS 记录 |
| 缓存与函数 | 缓存管理、边缘函数 |
...
- **手机端抽屉**：高度统一 `80vh`；**电脑端大型弹窗**：与文件预览同尺寸（`--vs-desktop-modal-*`，约 1120×860），头尾固定、内容区滚动
- **概览**（`index.php`）：自定义筛选（抽屉/弹窗 + 标签）+ 侧栏 6 项 KPI（sparkline）+ L7 流量多维 Tab 折线图 + Top 排行
- **站点管理**（`zones.php`）：站点数据概览（4 KPI + 指标图 Tab）+ 官方式站点列表与搜索；新增站点抽屉（仅未绑定套餐）
- **域名管理**（`domains.php`）：站点选择 + 站点信息同一板块；官方式加速域名列表；CNAME 复制；HTTPS 配置抽屉（免费证书）；抽屉添加/编辑/启停/删除
- **规则引擎**（`rules.php`）：站点加速规则列表；拖拽调整优先级；启用/编辑/复制/删除；创建抽屉（HOST 匹配 + 常用操作）
