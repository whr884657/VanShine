...
- **概览**（`index.php`）：统一筛选（时间 1 分钟~31 天 / 站点 / 域名）+ 多指标折线图墙；全站点默认分线 + 总计；套餐流量/请求配额 + 刷新预热配额
- **数据分析**（`analytics.php`）：可选指标、时间范围、粒度，KPI + 折线图
- **套餐计费**（`billing.php`）：套餐卡片、绑定站点、今日/本月用量与计费趋势
- EdgeOne 内页切换、查询表单、切换站点为**局部刷新**（`?fragment=1`），不整页 reload
