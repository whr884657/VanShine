 ...
 * 作用：VanShine Web 五步安装向导（单文件实现全部安装逻辑）
 * @version 1.0.5
 */
...
function writeInstallLock()
{
    ...
    $content = date('Y-m-d H:i:s') . ' | VanShine v' . VS_VERSION . "\n";
    if (file_put_contents($file, $content) === false) {
...
<div class="vs-page vs-install-page">
    <div class="vs-container">
        <div class="vs-install-header">
            <h1 class="vs-install-title">VanShine 安装向导</h1>
        ...
        <div class="vs-card vs-install-card">
            ...
            <?php if ($step === 1): ?>
                ...
                <p class="vs-card-desc">系统将检测服务器环境是否满足 VanShine 运行要求。</p>
            ...
            <?php elseif ($step === 3): ?>
                ...
                <?php else: ?>
                    <p class="vs-card-desc">数据库为空，可以直接创建 VanShine 所需的数据表。</p>
            ...
            <?php elseif ($step === 5): ?>
                <div class="vs-success-block">
                    ...
                    <p class="vs-card-desc">VanShine v<?php echo vs_e(VS_VERSION); ?> 已成功安装，您可以开始使用了。</p>
        ...
        <div class="vs-install-footer">
            <span>VanShine &copy; <?php echo date('Y'); ?> v<?php echo vs_e(VS_VERSION); ?></span>
        </div>
    </div>
</div>
