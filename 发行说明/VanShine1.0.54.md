...
修复分享短链接 404、弹窗遮挡与移动端分享管理体验；后台文件模块目录结构调整。
...
- **链接格式不变**：`https://域名/d/{32位token}`，无需 `?` 参数（预览/下载走 `/d/{token}/stream?file=ID`）

### 预览与分享 UX

- 创建分享弹窗 z-index 高于预览层，不再被遮挡
- 同一文件多条分享：显示「已创建 N 条」可展开查看
- 过期时间：date + time 组合控件（替代原生 datetime-local）
- 「文件信息与操作」折叠区样式优化

### 分享管理（手机端）

- ≤768px 自动切换为卡片布局：链接在上，密码/过期/状态在下

### 目录结构

- `admin/files/index.php` — 文件管理
- `admin/files/shares.php` — 分享管理
- 旧地址 `/admin/files.php`、`/admin/shares.php` 301 跳转
